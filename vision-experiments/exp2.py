...
from datasets_mnist import dataloaders_2
import models_nn
...
def reconfig(general_config, model_config, data_config):
	"""
	Re-initialization for model_config and data_config based on requirements of experiments 2.
	"""
...
def exp2_nn(general_config, model_config, data_config, dataset_tr, dataset_te, verbose):
	...
	if verbose:
		...
		print("\tThere are {} splits used for retraining splits - they are added with the remaining training data from label {} evenly. (ReLearning)".format(dataloaders.n_relearning, label))
