...
for i in range(100):
    ...
    for i, instance in enumerate(tqdm(inst_test)):
        ...
        for attr in label:
            ...
            samples = 100
            obj0_p = tree.predict_probs_mixture(instance, samples, False, False, 0)[attr]
            obj0_v = sorted([(obj0_p[val], val) for val in obj0_p])[-1][1]

            obj0_greedy_p = tree.predict_probs_mixture(instance, samples, True, False, 0)[attr]
            obj0_greedy_v = sorted([(obj0_greedy_p[val], val) for val in obj0_greedy_p])[-1][1]
            ...
            obj0_acc.append(int(obj0_v == label[attr]))
            obj0_greedy_acc.append(int(obj0_greedy_v == label[attr]))
    ...
    train_rows_with_nan = train_encoded.isna().any(axis=1)

    train_encoded = train_encoded[~train_rows_with_nan]
    y_train_nb = y_train.reset_index(drop=True)[~train_rows_with_nan]

    test_rows_with_nan = test_encoded.isna().any(axis=1)
    if (test_rows_with_nan.any()):
        print("NB **DROPPING {} NA ROWS**".format(test_rows_with_nan.sum()))

    test_encoded = test_encoded[~test_rows_with_nan]
    y_test_nb = y_test.reset_index(drop=True)[~test_rows_with_nan]

    # print(rows_with_nan)

    clf.fit(train_encoded, y_train_nb)
    # print(y_train)
    y_h = clf.predict(test_encoded)
    # print(y_h)
    # print(y_test.to_numpy().flatten())
    NB.append(y_h == y_test_nb.to_numpy().flatten())
...
print('NAIVE BAYES:', np.concatenate(NB).mean())
print('DECISION TREE:', np.concatenate(DT).mean())
