...
def experiment_1_nn(general_config, model_config, data_config, dataset_tr, dataset_te, verbose):

	reconfig(general_config, model_config, data_config)
	label = general_config['label']

	# Dataloaders:
	dataloaders = dataloaders_1(general_config, data_config, dataset_tr, dataset_te, verbose=verbose)
	...
	if verbose:
		...
		print("Experiments description: Train all data from label {} first, then train the rest with sequential splits.".format(label))
...
def experiment_1_cobweb(general_config, model_config, data_config, dataset_tr, dataset_te, verbose):
