...
exp1$TrainSet <- factor(exp1$TrainSet, levels = c('L0', 'S1', 'S2', 'S3', 'S4', 'S5', 'S6', 'S7', 'S8', 'S9'))
...
p <- ggplot(df, aes(x = TrainSet, y = TestAccuracy, color = TestSet, fill = TestSet, group = TestSet)) +
  #geom_line() +
  ...
  stat_summary(fun.data = function(y) data.frame(y = median(y)),
               ...
               size = 1,
               alpha = 0.5) +
  labs(x = 'Incoming Training Split', y = 'Test Accuracy', title = 'Test Accuracy with More Pre-defined Training Splits, Experiment 1') +
  theme_minimal() +
  facet_wrap(~ Model, ncol = 3) +
  scale_color_manual(values = c('Chosen' = 'red', 'Rest' = 'darkgreen', 'All' = 'blue')) +
  scale_fill_manual(values = c('Chosen' = 'red', 'Rest' = 'darkgreen', 'All' = 'blue')) +
...
p_chosen <- ggplot(df_chosen, aes(x = TrainSet, y = TestAccuracy, color = Model, fill = Model, group = Model)) +
  ...
  stat_summary(#fun.data = function(y) data.frame(y = median(y)),
               ...
               size = 1,
               alpha = 0.5) +
  labs(x = 'Incoming Training Split', y = 'Test Accuracy', title = 'Test Accuracy for Chosen Label with More Pre-defined Training Splits, Experiment 1') +
  theme_minimal() +
  #facet_wrap(~ Model, ncol = 3) +
