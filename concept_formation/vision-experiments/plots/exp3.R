...
exp3$TrainSet <- factor(exp3$TrainSet, levels = c('S0', 'S1', 'S2', 'S3', 'S4', 'R1', 'R2', 'R3', 'R4', 'R5'))
...
p_chosen <- ggplot(df_chosen, aes(x = TrainSet, y = TestAccuracy, color = Model, fill = Model, group = Model)) +
  ...
  stat_summary(#fun.data = function(y) data.frame(y = median(y)),
    ...
    size = 1,
    alpha = 0.5) +
  labs(x = 'Incoming Training Split', y = 'Test Accuracy', title = 'Test Accuracy for Chosen Label with More Pre-defined Training Splits, Experiment 3') +
  theme_minimal() +
