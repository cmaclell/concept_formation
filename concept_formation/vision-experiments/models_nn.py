...
class FC_CNN(nn.Module):
	def __init__(self, available_labels=10, kernel_size=5):
...
class FC(nn.Module):
	def __init__(self, n_hidden, n_nodes, available_labels=10, image_size=28):
		...
		self.fc_out = nn.Linear(n_nodes, 10)
...
def build_model(model_config, data_config, device):
	...
	if model_config['type'] == 'fc':
		model = FC(n_hidden=n_hidden, n_nodes=n_nodes, available_labels=available_labels, image_size=image_size).to(device)
	else:
		model = FC_CNN(available_labels=available_labels, kernel_size=kernel_size).to(device)
	# if cuda:
	# 	model = model.cuda()
