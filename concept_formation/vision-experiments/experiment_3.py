...
from datasets_mnist import dataloaders_3
import models_nn
...
def experiment_3_nn(general_config, model_config, data_config, dataset_tr, dataset_te, verbose):
	...
	# Dataloaders:
	dataloaders = dataloaders_3(general_config, data_config, dataset_tr, dataset_te, verbose=verbose)
	...
	if verbose:
		...
		print("\tIn the last {} splits, fit in the remaining training data from label {} evenly. (ReLearning)".format(dataloaders.n_relearning, label))
...
def experiment_3_cobweb(general_config, model_config, data_config, dataset_tr, dataset_te, verbose):
